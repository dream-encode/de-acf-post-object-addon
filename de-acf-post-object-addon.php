...
class DE_ACF_Post_Object_Addon_Plugin {
		...
		$this->settings_fields = array(
			...
			'append_field' => array(
				...
				'args' => array(
					...
					'hint' => 'This field will be displayed after the post title.',
					'toggled_by' => 'enable',
					'class' => array(
						'select2',
					),
			...
			'append_field_format' => array(
				...
				'args' => array(
					...
					'hint' => 'This is how the append field will be formatted.',
					'toggled_by' => 'enable',
					'class' => array(
						'select2',
					),
	...
	function load_setting_sections() { 
		...
		$this->sections['Settings'] = array(
			'name' => 'Settings',
			'description' => 'Settings for ACF Post Object Field Type plugin.  This plugin is specfically for appending post data to each option for the "post object" field type.  When enabled, the selected fields will be suffixed to each available option in "post object" selects.',
			'id' => 'settings',
	...
	function plugin_action_links( $links ) {
		 $links[] = '<a href="'. esc_url( menu_page_url( $this->plugin_key, false ) ) .'">' . __('Settings', 'General') . '</a>';
	...
	function add_action_plugin( $actions, $plugin_file ) {
		...
		if ($plugin == $plugin_file) {
			$settings = array(
				'settings' => '<a href="'. esc_url( menu_page_url( $this->plugin_key, false ) ) .'">' . __('Settings', 'General') . '</a>'
			);
	...
	function get_acf_append_field_format_options() {
		$format_options = array(
			'wrap::par' 				=> __('Post Title &lpar;Append Field Value&rpar;', $this->lang_domain),
			'separator::dash' 	=> __('Post Title &dash; Append Field Value', $this->lang_domain),
			'separator::colon' 	=> __('Post Title&colon; Append Field Value', $this->lang_domain),
			'separator::gt' 		=> __('Post Title &gt; Append Field Value', $this->lang_domain),
			'separator::raquo' 	=> __('Post Title &raquo; Append Field Value', $this->lang_domain),
			'separator::gg' 		=> __('Post Title &gg; Append Field Value', $this->lang_domain),
			'separator::verbar' => __('Post Title &verbar; Append Field Value', $this->lang_domain),
			'wrap::brack' 			=> __('Post Title &lbrack;Append Field Value&rbrack;', $this->lang_domain),
			'wrap::brace' 			=> __('Post Title &lbrace;Append Field Value&rbrace;', $this->lang_domain),
		);
	...
	function settings_section_callback($args) {
		echo __('<p>'.$this->sections[$args['title']]['description'].'</p>', $this->lang_domain);
	}
...
?>
    ...
    <div class="wrap"> 
      ...
      <form method="post" action="options.php" id="<?php echo $this->plugin_key; ?>-form">
        ...
        <p class="submit">      
        	<?php submit_button( 'Save Changes', 'primary', $this->plugin_key.'-submit', false, array( 'id' => $this->plugin_key.'-submit' ) ); ?>&nbsp;&nbsp;<img src="/wp-admin/images/spinner.gif" id="<?php echo $this->plugin_key; ?>-ajax-working" class="hidden">
        </p>
      </form> 
    </div>
